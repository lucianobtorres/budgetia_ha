...
const TooltipContent = ({ children, className }: { children: React.ReactNode, className?: string }) => (
  <div className={cn(
    "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max px-2 py-1 text-xs text-text-primary bg-surface-card border border-border rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50",
    className
  )}>
    {children}
  </div>
