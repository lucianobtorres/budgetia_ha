...
class LearnFactInput(BaseModel):
    category: str = Field(
        ...,
        description="Category of the fact (e.g., 'preference', 'goal', 'transporte', 'alimentacao').",
    )
    fact: str = Field(..., description="The information to remember about the user.")


class UpdateFactInput(BaseModel):
    old_fact_snippet: str = Field(
        ..., description="A unique snippet of the OLD fact to be replaced."
    )
    new_fact: str = Field(..., description="The NEW updated fact.")


class ForgetFactInput(BaseModel):
    fact_snippet: str = Field(
        ..., description="A unique snippet of the fact to forget."
    )
