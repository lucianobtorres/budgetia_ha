...
class WhatsAppChannel(INotificationChannel):
    ...
    def __init__(self):
        ...
        if not self.account_sid or not self.auth_token:
            logger.warning(
                "Credenciais do Twilio não configuradas. WhatsAppChannel desativado."
            )
            self.client = None
        else:
            try:
    ...
    async def send(self, recipient_id: str, message: NotificationMessage) -> bool:
        if not self.client:
            logger.debug(
                "Tentativa de envio sem cliente Twilio configurado (Mock/Skipped)."
            )
            return False
        ...
        try:
            ...
            msg = self.client.messages.create(
                from_=self.from_number, body=full_message, to=recipient_id
            )
