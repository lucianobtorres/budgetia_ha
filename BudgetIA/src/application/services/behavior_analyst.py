...
class BehaviorAnalyst:
    ...
    def __init__(
        self, llm_orchestrator: LLMOrchestrator, memory_service: MemoryService
    ):
        self.llm = llm_orchestrator.get_configured_llm()
        self.memory = memory_service

    def analyze_recent_transactions(
        self, transactions_df: pd.DataFrame, days: int = 60
    ) -> list[str]:
        """
        ...
        try:
            messages = [
                SystemMessage(
                    content="Você é um componente de backend que analisa dados financeiros e retorna JSON estruturado."
                ),
            ...
            for item in facts_data:
                ...
                if description:
                    self.memory.add_fact(
                        category, description, source="behavior_analyst", metadata=meta
                    )
