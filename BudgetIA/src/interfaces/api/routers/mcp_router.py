...
async def sse(
    ...
    """
    scope = request.scope
    # No FastAPI/Starlette, receive e send estão disponíveis no request
    receive = request.receive
    send = request._send  # Atributo interno do Starlette usado para resposta
...
async def mcp_rpc(
    ...
    try:
        ...
        logger.debug(
            f"MCP RPC: Recebida requisição de '{user_id}': {body.get('method')}"
        )
                ...
                try:
                    if method == "tools/list":
                        ...
                        resp = await session.send_request(
                            JSONRPCRequest(method=method, params=params, id=request_id)
                        )
                    ...
                    logger.error(
                        f"MCP RPC: Erro na chamada do método '{method}': {call_e}"
                    )
                    final_response = {
                        "jsonrpc": "2.0",
                        "error": {"code": -32603, "message": str(call_e)},
                        "id": request_id,
                    }
                finally:
...
async def messages(request: Request, config_service=Depends(get_user_config_service)):
    """
