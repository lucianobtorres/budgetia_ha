...
class ChatResponse(BaseModel):
    ...
    google_files_list: list[dict[str, Any]] | None = None  # List of {id, name}
...
def chat(
    ...
    response_text = orchestrator.process_user_input(
        input_data.text, extra_context=input_data.context
    )
    ...
    if "[UI_ACTION:show_file_selection]" in response_text:
        try:
            files_list = (
                orchestrator.get_google_auth_service().list_google_drive_files()
            )
...
async def upload_file(
    ...
    try:
        ...
        orchestrator._context["uploaded_file"] = MockFile(
            save_path, file.filename or "uploaded_file"
        )
...
def get_google_auth_url(
    ...
    url = auth_service.generate_authorization_url(
        current_onboarding_state=current_state, redirect_uri=redirect_uri
    )
    return {"url": url}
