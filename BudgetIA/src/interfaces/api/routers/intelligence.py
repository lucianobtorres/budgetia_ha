...
from interfaces.api.schemas.intelligence import (
    ObserverInfoSchema,
    SubscriptionKeywordsUpdate,
    ToolInfoSchema,
)
...
# Modelos movidos para schemas/intelligence.py
...
def get_available_tools():
    ...
    ESSENTIAL_TOOLS = [
        ...
        "view_data_tool",
        "calculate_balance_tool",
        "check_budget_status_tool",
        "delete_transaction_tool",
        "update_transaction_tool",
        "generate_monthly_summary_tool",
        "register_ai_insight_tool",
        "define_budget_tool",
    ]
                    ...
                    for attribute_name in dir(module):
                        ...
                        if (
                            ...
                            if name in TOOL_TRANSLATIONS:
                                ...
                                label = getattr(
                                    tool_class, "label", name.replace("_", " ").title()
                                )
...
@router.get("/observers", response_model=list[ObserverInfoSchema])
def get_active_observers(
    ...
    try:
        ...
        logger.info(
            f"OBSERVERS: Regras padrão carregadas: {[r.rule_name for r in rules]}"
        )
        ...
        OBSERVER_TRANSLATIONS = {
            ...
            "expense_anomaly": "Detector de Anomalias de Valor",
            "semantic_anomaly": "Detector de Anomalias Semânticas",
            ...
            "SemanticAnomalyRule": "Detector de Anomalias Semânticas",
        }
        ...
        for r in rules:
            try:
                ...
                if not display_name:
                    ...
                    base_name = OBSERVER_TRANSLATIONS.get(
                        r.rule_name, r.rule_name.replace("_", " ").title()
                    )
                ...
                observers.append(
                    ObserverInfoSchema(
                        id=r.rule_name,
                        name=display_name,
                        description=r.__doc__.strip()
                        if r.__doc__
                        else "Monitoramento financeiro inteligente.",
                        is_active=True,
...
async def update_subscription_keywords(
    ...
    try:
        user_config_service.save_comunicacao_field(
            "subscription_keywords", update.keywords
        )
        return {
            "status": "success",
...
async def trigger_learning(
    user_config_service: UserConfigService = Depends(get_user_config_service),
    llm_orchestrator: Any = Depends(
        lambda: None
    ),  # TODO: Fix Dependency Injection for LLM
