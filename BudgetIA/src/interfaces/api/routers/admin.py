...
from interfaces.api.utils.security import get_user, load_users, save_users
...
def impersonate_user(req: ImpersonateRequest, admin: dict = Depends(verify_admin_role)):
    ...
    if target.get("role") == "admin":
        raise HTTPException(
            status_code=403, detail="Não é permitido impersonar outro administrador."
        )

    access_token = create_access_token(
        data={"sub": req.target_username}, expires_delta=timedelta(minutes=60)
    )
...
def toggle_block_endpoint(username: str, admin: dict = Depends(verify_admin_role)):
    ...
    return {
        "message": "Bloqueado" if new_state else "Desbloqueado",
...
def reset_password_endpoint(
    username: str, req: ResetPasswordRequest, admin: dict = Depends(verify_admin_role)
):
...
class BroadcastRequest(BaseModel):
    ...
    priority: str = "normal"
    target_usernames: list[str] | None = None  # None or Empty = All
    ...
    for username in targets:
        try:
            ...
            push_service = PushNotificationService(
                config_service.config_dir
            )  # Reusing dependency logic
            ...
            if req.send_notification:
                notif_service.add_notification(
                    message=req.message, category=req.category, priority=req.priority
                )
                notif_count += 1
